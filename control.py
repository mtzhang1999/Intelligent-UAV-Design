...
# run roslaunch uav_sim windows.launch, then run rostopic pub /tello/cmd_start std_msgs/Bool "data: 1"
...
class ControllerNode:
    ...
    def __init__(self):
        ...
        # pose in the world coordinates
        self.R_wu_ = R.from_quat([0, 0, 0, 1])
        self.t_wu_ = np.zeros([3], dtype=np.float64)
	    self.window_index = 0
        self.count = 0
        self.image_ = None
        self.color_range_ = [(0, 43, 46), (6, 255, 255)] # HSV of RED
    ...
    def decision(self):
        if self.flight_state_ == self.FlightState.WAITING:  # 起飞并飞至离墙体（y = 3.0m）适当距离的位置
            ...
            if self.detectTarget():
                ...
                self.window_index = win_index
                if win_index == 0 or win_index == 1:
                    self.navigating_queue_ = deque([['y', 2.4], ['z', 1.0], ['x', self.window_x_list_[win_index]],  ['y', 4.0], ['x', 2.5], ['y',6],['x',1.5],['y', 7.5], ['z', 0.72]])  # 通过窗户并导航至终点上方
                else:                    
                    self.navigating_queue_ = deque([['y', 2.4], ['z', 1.0], ['x', self.window_x_list_[win_index]], ['y', 6.0], ['x',1.5],['y', 7.5], ['z', 0.72]]) # 通过窗户并导航至终点上方

                    
                self.switchNavigatingState()
                self.next_state_ = self.FlightState.DETECTING_1
            else:
                if self.t_wu_[0] > 7.5:
        ...
        elif self.flight_state_ == self.FlightState.DETECTING_1:
            rospy.logwarn('DETECTING_1')
            (yaw, pitch, roll) = self.R_wu_.as_euler('zyx', degrees=True)
	    if self.count < 3:
	    	self.publishCommand('cw %d' % (int(30)))
		self.count = self.count + 1
	    else:
		self.count = 0
		self.navigating_queue_ = deque([['y', 13.0], ['x', 3.8], ['z', 1.60]])
	        self.switchNavigatingState()
		self.next_state_ = self.FlightState.DETECTING_2

	elif self.flight_state_ == self.FlightState.DETECTING_2:
	    rospy.logwarn('State:DETECTING_2')
	    self.publishCommand('cw %d' % (int(180)))

	    self.navigating_queue_ = deque([['x', 6.5], ['y', 9.5], ['z', 1.0]])
	    self.switchNavigatingState()
   	    self.next_state_ = self.FlightState.DETECTING_3
	elif self.flight_state_ == self.FlightState.DETECTING_3:
	    rospy.logwarn('State:DETECTING_3')
	    self.publishCommand('ccw %d' % (int(90)))
	    self.navigating_queue_ = deque([['z', 1.72]])
	    self.switchNavigatingState()
	    self.next_state_ = self.FlightState.DETECTING_4
	elif self.flight_state_ == self.FlightState.DETECTING_4:
	    rospy.logwarn('State:DETECTING_4')
	    self.publishCommand('ccw %d' % (int(180)))
	    self.navigating_queue_ = deque([['y', 14.5], ['x', 6]])
	    self.switchNavigatingState()
